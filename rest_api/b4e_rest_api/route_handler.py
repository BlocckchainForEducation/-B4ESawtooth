...
class RouteHandler(object):
    ...
    async def create_teachers(self, request):
        ...
        list_transaction_id = await self._messenger.send_create_teachers(private_key=body.get('privateKeyHex'),
                                                                         profiles=profiles,
                                                                         timestamp=get_time())
        list_teachers = tolist(slice_per(profiles, 1000))
        transactions = []
    ...
    async def create_edu_officers(self, request):
        ...
        list_edu_officers = tolist(slice_per(profiles, 1000))
        transactions = []
        for i in range(len(list_transaction_id)):
            transactions.append({
    ...
    async def create_classes(self, request):
        ...
        list_classes = tolist(slice_per(body.get('classes'), 1000))
        transactions = []
        for i in range(len(list_transaction_id)):
            transactions.append({
    ...
    async def create_subjects(self, request):
        ...
        list_subjects = tolist(slice_per(body.get('points'), 1000))
        transactions = []
        class_id = body.get('classId')
        for i in range(len(list_transaction_id)):
            transactions.append({
    ...
    async def create_certs(self, request):
        ...
        list_certs = tolist(slice_per(body.get('certificates'), 1000))
        transactions = []
        for i in range(len(list_transaction_id)):
            transactions.append({
